...
class SSFTTnet(nn.Module):
    def __init__(
        ...
        self.conv2d_features = nn.Sequential(
            nn.Conv2d(
                in_channels=kennel_3D * 28, out_channels=kennel_2D, kernel_size=(3, 3)
            ),
        ...
        self.token_wA = nn.Parameter(
            torch.empty(1, self.L, kennel_2D), requires_grad=True
        )  # Tokenization parameters
        torch.nn.init.xavier_normal_(self.token_wA)
        self.token_wV = nn.Parameter(
            torch.empty(1, kennel_2D, self.cT), requires_grad=True
        )  # Tokenization parameters
