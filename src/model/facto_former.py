...
class Attention(nn.Module):
    ...
    def __init__(self, dim, n_heads=4, qkv_bias=True, attn_p=0.0, proj_p=0.0):
        super().__init__()
...
class PatchEmbed3D(nn.Module):
    ...
    def forward(self, x):
        B, T, H, W = x.shape
        assert (
            H == self.img_size[1] and W == self.img_size[2]
...
class FactoFormer(nn.Module):
    ...
    def __init__(
        ...
        self.spatial_patch_embed = PatchEmbed3D(
            ...
            stride=spatial_patch,
        )
        self.spectral_patch_embed = PatchEmbed3D(
            img_size=img_size,
            patch_size=spectral_patch,
            embed_dim=spectral_embed_dim,
            stride=spectral_patch,
        )
        ...
        self.spatial_pos_embed = nn.Parameter(
            torch.zeros(1, 1 + num_patches_spatial, spatial_embed_dim)
        )
        self.spectral_pos_embed = nn.Parameter(
            torch.zeros(1, 1 + num_patches_spectral, spectral_embed_dim)
        )
        ...
        self.spectral_transformer = Transformer(
            dim=spectral_embed_dim,
            depth=depth,
            n_heads=n_heads,
            mlp_ratio=4,
            qkv_bias=qkv_bias,
