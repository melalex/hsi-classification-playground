...
def get_3d_sincos_pos_embed(
    embed_dim, t_size, grid_size, cls_token=False, scale_t=None
):
...
class Attention(nn.Module):
    ...
    def adjust_drop_rate(self, drop_rate=0.0):
...
class SwiGLU(nn.Module):
    def __init__(self, dim, hidden_dim, multiple_of=4, dropout=0.0):
        super().__init__()
        hidden_dim = int(
...
class HSIMAE(nn.Module):
    ...
    def initialize_weights(self):
        pos_embed = get_3d_sincos_pos_embed(
            self.dim, self.input_size[0], self.input_size[1]
        )
        self.pos_embed.data.copy_(pos_embed)
        decoder_pos_embed = get_3d_sincos_pos_embed(
            self.dec_dim, self.input_size[0], self.input_size[1]
        )
    ...
    def spatial_spectral_masking(self, x, T, L, mask_ratio):
        ...
        mask_all = (
            mask_1
            + mask_2
            + torch.linspace(0, 0.5, T * L, device=x.device).unsqueeze(0).repeat(N, 1)
        )
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        x, mask, ids_restore, ids_keep = self.spatial_spectral_masking(
            x, T, L, mask_ratio
        )
        x = x.view(N, -1, C)
