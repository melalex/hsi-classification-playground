...
class FFN(nn.Module):
    def __init__(
        ...
        self.Conv1 = nn.Conv2d(
            in_features, hidden_features, kernel_size=(1, 1), bias=False
        )
        self.act = act_layer()
        ...
        self.Conv2 = nn.Conv2d(
            hidden_features, out_features, kernel_size=(1, 1), bias=False
        )
...
class MDCP(nn.Module):
    def __init__(
        ...
        self.down_sample = nn.Conv2d(
            embed_dim, embed_dim, kernel_size=(3, 3), stride=(2, 2)
        )
        self.weights = nn.Parameter(torch.ones(2))
...
class Add2D(nn.Module):
    def __init__(self, in_channels, kernel_size, padding, groups_s):
        ...
        self.conv3 = nn.Conv2d(
            in_channels, in_channels, kernel_size=1, padding=0, groups=1
        )
...
class DCTN(nn.Module):
    ...
    def __init__(
        ...
        self.conv4_2 = nn.Conv2d(
            embed_dims[3], embed_dims[3], kernel_size=3, padding=1, groups=embed_dims[3]
        )
    ...
    def forward(self, x):
        ...
        x = self.ConvEnd(
            torch.concat([x_c4, x.permute(0, 3, 1, 2), x_embedding], dim=1)
        )
        x = x.permute(0, 2, 3, 1)
        B, H, W, C = x.shape
        x = x.reshape(B, -1, C)
